...
class StorageService {
  ...
  Future<List<String>> uploadEquipmentImages(String equipmentId, List<File> imageFiles) async {
    try {
      final List<String> uploadedUrls = [];

      for (int i = 0; i < imageFiles.length; i++) {
        final String fileName = '${equipmentId}_$i.jpg';
        final Reference ref = _storage.ref().child('equipment_images').child(fileName);
  ...
  Future<List<String>> uploadEquipmentImagesWeb(String equipmentId, List<Uint8List> imageBytes) async {
    try {
      final List<String> uploadedUrls = [];
